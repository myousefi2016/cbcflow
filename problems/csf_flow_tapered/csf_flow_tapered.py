...
def get_pulse_input_function(V, z_index, factor, A, HR_inv, HR, b, f1):
	...
	vel[z_index] = v_z
        ...
        defaults = {"factor":factor, "A":A, "p0":1, "vd":0.03, "Ees":50, "T": HR_inv, "HR":HR, "rad":rad, "b":b, 'f1':f1, 't':0.0}
...
def get_sine_input_function(V, z_index, HR, HR_inv, v_max):
	...
	vel = ["0.0", "0.0", "0.0"]
	vel[z_index] = v_z
        defaults = {'HR':HR, 'v_max':v_max, 'T':HR_inv, 't':0.0}
...
#		self.characteristic = "preassure at node "+ str(self.pressure_index) + " at peak velocity"
	...
	def boundary_conditions(self, V, Q, t):
		...
		if self.waveform == "pulse":
			self.g1 = get_pulse_input_function(V, self.z_index, self.factor1, self.A, self.HR_inv, self.HR, self.b, self.f1)
    			self.g1.t = t 
			self.g2 = get_pulse_input_function(V, self.z_index, self.factor2, self.A, self.HR_inv, self.HR, self.b, self.f1)
			self.g2.t = t

		elif self.waveform =="sine":
			self.g1 = get_sine_input_function(V, self.z_index, self.HR, self.HR_inv, self.v_max1)
			self.g1.t = t 
			self.g2 = get_sine_input_function(V, self.z_index, self.HR, self.HR_inv, self.v_max2)
			self.g2.t = t
