...
class IPCS_Stable_NonNewtonian(NSScheme):
    ...
    def solve(self, problem, update, timer):
        ...
        mu = modifiedcross(u1)
        problem.params.mu = mu # FIXME: Redesign so the problem can return the
        ...
        for timestep in xrange(start_timestep+1,len(timesteps)):
            ...
            # Update viscosity
            mu.assign(modifiedcross(u1)) # FIXME: Redesign so this can be done in problem.update()?
