...
from ..core.schemeutils import assign_ics_split, make_velocity_bcs, make_pressure_bcs, make_rhs_pressure_bcs
from ..core.spaces import NSSpacePoolSplit

from time import time


class CoupledPreconditoned(NSScheme):
    ...
    def solve(self, problem, update):
        ...
        #AA = block_assemble([[1/k*mu+nu*au+cu, b1],[b2, 0]], bcs=bcs)
        #AA = block_assemble([[fu, b1],[b2, 0]], bcs=bcs)
        ...
        AA = block_assemble([[fu, theta*b1],[theta*b2, 0]], bcs=[bcu, bcp])
        ...
        for timestep in xrange(start_timestep+1,len(timesteps)):
            ...
            t1 = time()
            TOL = 1e-6
            AAinv = LGMRES(AA, precond=P_inv, tolerance=TOL, maxiter=50, show=3, initial_guess=guess)
