...
class RestartInfo(object):
    ...
    def _load_at_time(self, t, functionspace, name):
        ...
        if len(filenames) != 1:
            headflow_print(str(filenames))
            raise RuntimeError("Found more than one filename.")
        regexp = re.compile("_at_t%d_(.*).xml.gz"%self.timestep)

        # FIXME: Should support going back in time, using linear interpolation
        # between saved files.
        if t != self.t:
            headflow_warning("Using data for current time %.2g instead of %.2g for %s" % (self.t, t, name))
