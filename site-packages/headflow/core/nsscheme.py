...
class NSScheme(Parameterized):
    ...
    def _postproc_desegregate(self, u):
    ...
    def _check_runaway_solution(self, problem, u):
        if self._segregated:
    ...
    def _postproc(self, problem, u, p, t): # CLEANUP: Move to postproc
        self._postproc_save_solution(problem, u, p, t)
        self._postproc_save_final_solution(problem, u, p, t)
        self._postproc_save_vectors(problem, u, p, t)
        self._postproc_plot(problem, u, p, t)

    def _postproc_save_solution(self, problem, u, p, t): # CLEANUP: Move to postproc
        # Save solution
        if self.params.save_solution and (self._timestep - 1) % self.params.save_frequency == 0:
    ...
    def _postproc_save_final_solution(self, problem, u, p, t): # CLEANUP: Move to postproc
        # Save solution at t = T
        if self.params.save_solution_at_final_t and t >= problem.T:
            casedir = self.params.casedir
    ...
    def _postproc_save_vectors(self, problem, u, p, t): # CLEANUP: Move to postproc
        # Save vectors in xml format
        if self.params.save_xml and (self._timestep - 1) % self.params.save_frequency == 0:
            casedir = self.params.casedir
    ...
    def _postproc_plot(self, problem, u, p, t): # CLEANUP: Move to postproc
        if self.params.plot_solution:
            plot(self._postproc_desegregate(u), title="Velocity", rescale=True)
    ...
    def update(self, problem, t, u, p):
        ...
        # Validation (TODO: where does this belong?)
        self._check_runaway_solution(problem, u)

        ################## CLEANUP: We want this part:
        # Update problem for this timestep
        problem.update_problem(t, u, p)
        self._t.append(t)


        ################## CLEANUP: Move to postproc
        self._postproc(problem, u, p, t)
        ...
        if master:
            ...
            mm, ss = divmod(ss, 60)
            print
            msg = "Time step %d finished in %.2f seconds, %.1f%% done (t=%.3g, T=%g; %02d:%02d:%02d remaining)." \
        ...
        ################## CLEANUP: We might want this part?
        # Increase time step
        self._timestep += 1
