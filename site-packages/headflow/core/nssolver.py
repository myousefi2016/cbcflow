...
from ..dol import plot, parameters, as_vector
...
class NSSolver(Parameterized):
    ...
    def _summarize(self):
        ...
        msg = "Total time spent in NSSolver: %s" % time_to_string(self._final_time - self._initial_time)
        headflow_print(msg)
    ...
    def _update_timing(self, timestep, t, time_at_top):
        ...
        msg = ("Timestep %5d finished (t=%.2e, %.1f%%) in %3.1fs (solve: %3.1fs). Time spent: %s Time remaining: %s" \
                                       % (timestep, t,
                                          ...
                                          time_to_string(self._accumulated_time*(self.problem.params.T-t)/t),
                                          ))
    ...
    def _update_plot(self, u, p):
        if self.params.plot_solution:
            if self.scheme._segregated:
                ...
                try:
                    plot(as_vector(u), title="Velocity")
                except:
