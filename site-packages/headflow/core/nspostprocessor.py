...
class NSPostProcessor(Parameterized):
    def __init__(self, params=None):
        ...
        self._dependencies = {}
        #self._reverse_dependencies = {} # TODO: Need something like this for proper time dependencies handling
        ...
        # TODO: This is not actually "compute times", but tied to "compute to perform actions",
        #       e.g. computing for a dependency does not count here. Rename?!
        self._previous_compute_time = defaultdict(lambda: -1e16)
        self._previous_compute_timestep = defaultdict(lambda: -1e16)
    ...
    def should_compute_at_this_time(self, field, t, timestep):
        ...
        pcts = self._previous_compute_timestep[field.name]
        if timestep - pcts < fp.stride_timestep:
            return False
        pct = self._previous_compute_time[field.name]
    ...
    def update_all(self, u, p, t, timestep, spaces, problem):
        ...
        timesteps_to_remember = sorted(self._cache.keys())[:-1]
        for tstep in timesteps_to_remember:
            keys_to_remember = self._cache[tstep].keys()
            for key in keys_to_remember:
                # FIXME: This will store references to the same functions... Must make copies!
                self._cache[tstep][key] = self._cache[tstep+1][key]

        #for tstep, key in self._items_to_cache:
        #    # FIXME: This will store references to the same functions... Must make copies!
        #    self._cache[tstep][key] = self._cache[tstep+1][key]
        ...
        for name in self._sorted_fields_keys:
            ...
            # TODO: Allow different time params per action?
            #       We can do that if we swap if and for here plus some details.
            #       That way we can e.g. store every 10 values but plot every value.
            if self.should_compute_at_this_time(field, t, timestep):
                ...
                self._previous_compute_time[field.name] = t
                self._previous_compute_timestep[field.name] = timestep
        ...
        # Get or compute last delta t
        #if timestep > 0: # TODO: Maybe inject a permanent dependency on Time at -1 above, then we can use this if.
        try: # TODO: Better to check cache instead of using exceptions for normal logic.
            dt = self.get("t", 0) - self.get("t", -1) 
        except:
        ...
        for name, dep in self._dependencies.items():
            ...
            for d in dep:
                ...
                cond2 = self.should_compute_at_this_time(field, t+abs(d[1])*dt, timestep+abs(d[1]))
