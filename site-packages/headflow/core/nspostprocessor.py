...
from dolfin import Function, MPI, plot, File, project, as_vector, HDF5File, XDMFFile, tic, toc
...
def safe_mkdir(dir):
    ...
    try:
        ...
        if not os.path.isdir(dir):
            # Silence exception only if we failed because directory was created by another process
            raise e
...
class NSPostProcessor(Parameterized):
    def __init__(self, params=None):
        ...
        #self._full_dependencies = {} # Indirect dependencies included TODO: Need this?
    ...
    def add_field(self, field):
        ...
        #if 1: print "DEBUG: Setting dependencies of %s to %s" % (field.name, deps)
        ...
        if data is None:
            if timestep == 0:
                ...
                if self._compute_counts[field.name] == 0:
                    field.before_first_compute(self, spaces, problem)
                ...
                if self._plan[0][name] > 0:
                    if isinstance(data, Function):
                        #if 1: print "DEBUG: Copying function for field", name
    ...
    def _update_hdf5_file(self, field_name, saveformat, data, save_count, t):
        ...
        # Delete (close) file object to avoid errors if program crashes
        del datafile
        return metadata
    ...
    def _action_save(self, field, data):
        ...
        for saveformat in save_as:
            ...
            if saveformat == 'pvd':
                ...
                metadata += self._update_hdf5_file(field_name, saveformat, data, save_count, t)
            else:
    ...
    def _rollback_plan(self, t, timestep):
        ...
        #print "RB:", "\n".join("%s = %s" % (k,self._plan[k]) for k in tss)
        #print ""
    ...
    def _update_plan(self, t, timestep):
        #if 1: print "DEBUG: In _update_plan"
        ...
        #if 1: print "DEBUG: Plan before rollback:", self._plan
        self._rollback_plan(t, timestep)
        #if 1: print "DEBUG: Plan after rollback:", self._plan
        ...
        #if 1: print "DEBUG: Triggered fields:", [name for name, field in triggered_fields]
        for name, field in triggered_fields:
            deps = self._dependencies[name]
            if deps:
                #if 1: print "DEBUG: Found dependencies, making plan for computing %s from %s" % (name, deps)
                ...
                for depname, ts in deps:
                    ...
                    #if 1: print "DEBUG: Addded plan[%s][%s] = %s" % (ts+offset, depname, ttk)
            else:
                #if 1: print "DEBUG: Found no dependencies, making plan for computing %s directly" % (name,)
    ...
    def _update_cache(self):
        #if 1: print "DEBUG: In _update_cache"
        ...
        for ts, plan in self._plan.iteritems():
            ...
            for name, ttk in plan.iteritems():
                if ttk > 0:
                    ...
                    #if 1: print "DEBUG: Retrieving from cache: %s, %s" % (name, ts)

                    # Insert data in new cache at position ts-1
                    new_cache[ts-1][name] = data
                    #if 1: print "DEBUG: Inserted in cache: %s, %s" % (name, ts-1)
        self._cache = new_cache

    def update_all(self, u, p, t, timestep, spaces, problem):
        "Update all PPFields"

        #if 1: print "DEBUG: In _update_all", t, timestep
