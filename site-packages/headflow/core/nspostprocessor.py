...
class NSPostProcessor(Parameterized):
    ...
    def _rollback_plan(self, t, timestep):
        ...
        for ts in sorted(self._plan.keys()):
            p = self._plan[ts]

            # Countdown time to keep each data item and remove what we want to forget
            for name, ttk in p.items():
                if ttk > 1:
                    p[name] = ttk - 1
                else:
                    del p[name]

            # Keep only the plan dicts with contents left
            tsp = ts - 1
            if p:
                self._plan[tsp] = p
            elif tsp in self._plan:
                del self._plan[tsp]
