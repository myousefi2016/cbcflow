...
def assign_ics_mixed(up0, spaces, ics):
    ...
    up0.assign(project2(up, spaces.W, name="up0_init_projection"))

def assign_ics_split(u0, p0, spaces, ics):
    u = as_vector(list(ics[0]))
    p = ics[1]
    u0.assign(project2(u, spaces.V, name="u0_init_projection"))
    p0.assign(project2(p, spaces.Q, name="p0_init_projection"))

def assign_ics_segregated(u0, p0, spaces, ics):
    for d in spaces.dims:
        u0[d].assign(project2(ics[0][d], spaces.U, name="u0_%d_init_projection"%d))
    p0.assign(project2(ics[1], spaces.Q, name="p0_init_projection"))
