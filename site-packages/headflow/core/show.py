...
def show_problem(problem, interactive=True, bc_snapshots=3):
    ...
    if problem.facet_domains is not None:
        plot(problem.facet_domains, title="Facet domains") # TODO: Change max value to something just above the other values
    ...
    for i in range(bc_snapshots):
        ...
        for j, bc in enumerate(bcu):
            print type(bc), len(bc)
            print [bc[k] for k in range(len(bc))]
            u, D = bc
