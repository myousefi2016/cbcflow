...
import dolfin
from .schemeutils import Spaces
from ..dol import *
...
def show_problem(problem, interactive=True, bc_snapshots=4):
    ...
    spaces = Spaces(problem.mesh, mode="split", u_degree=1, p_degree=1, u_family="CG", p_family="CG")
    V = spaces.V
    Q = spaces.Q
    ...
    for i, tv in enumerate(timesteps):
        ...
        for j, bc in enumerate(bcu):
            u, D = bc
            D = "boundary domain %d " if isinstance(D, int) else ""
