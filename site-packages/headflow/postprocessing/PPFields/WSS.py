...
def local_mesh_to_boundary_dofmap_cpp(F, V):
    ...
    namespace dolfin {
      static Array<int>& global_to_local_dofmapping(boost::shared_ptr<dolfin::FunctionSpace> F, boost::shared_ptr<dolfin::FunctionSpace> V, boost::shared_ptr<const dolfin::GenericDofMap> full_dofmap, boost::shared_ptr<const dolfin::GenericDofMap> bdry_dofmap) 
      {
        ...
        // Use intersection operator to find corresponding full mesh cell to boundary cell
        dolfin::IntersectionOperator iop(full_mesh);
            ...
            if(k==this_process) {
                for(size_t global_index=0; global_index < num_bdry_cells; global_index++) { 
                    ...
                    for(int i=0; i<3; i++) {
                        ...
                        int aie = iop.any_intersected_entity(p);
                        
                        if(aie >= 0) {
                            intersected_dofs[global_index] = full_dofmap->cell_dofs(aie);
                            break;
                        }
        ...
        */
        ...
        static Array<int> local_dofmapping(count*2);
    ...
    cpp_module = compile_extension_module(global_to_local_dofmapping)
