...
class PPField(Parameterized):
    ...
    @name.setter
    def name(self, value):
        if "Derivative" in self.name:
            self.__class__.__name__ = value
        else:
            raise AttributeError("Changing the name of %s is not allowed." %self.name)
...
class PPField(Parameterized):
        ...
        if self.params.saveparams.save:
            ...
            if MPI.process_number() == 0:
                if not isdir(self._savefolder):
                    ...
                    headflow_warning("Directory %s exists and is non-empty" %self._savefolder)
    ...
    def _save_data(self):
        ...
        if self._save_count == 0:
            ...
            if self.params.saveparams.saveformat == PPField.default_params().saveparams.saveformat:
