...
class PouseilleComponent(Expression):
    ...
    def eval(self, value, x):
        ...
        tp = self.transient_profile(self.t)

        # Output scaled value for this negative normal direction
        value[0] = -self.normal_component * self.scale_value * velocity_profile * tp
...
def make_pouseille_bcs(coeffs, mesh, indicator, scale_to=None, facet_domains=None):
    ...
    area = get_area(mesh, indicator, facet_domains) # This is the area of the _dicretized_ boundary
    radius = np.sqrt(area / pi) # FIXME: This is off by >10% in the pouseille2d demo!
    if 0:
        print "AREA:", area
        print "RADIUS:", radius
