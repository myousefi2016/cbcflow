...
class _WomersleyExpression(Expression):
    ...
    def set_t(self, t):
        self.t = t-np.floor(t/self.period)*self.period
    ...
    def get_sum_terms(self, x):
        try:
            sum_terms = self.all_sum_terms[(x[0],x[1],x[2])]
        except KeyError:
            sum_terms = np.zeros(self.N, dtype=np.complex)
            
            r = np.sqrt((x[0]-self.center[0])**2+(x[1]-self.center[1])**2+(x[2]-self.center[2])**2)
            y = r/self.a
            ...
            self.all_sum_terms[(x[0],x[1],x[2])] = sum_terms
...
class Womersley(BC_base, list):
    ...
    def __init__(self, coeffs, mesh, indicator, nu, scale_to=None):
        ...
        self.mesh = mesh
        self.ind = indicator
        dsi = ds[mesh.domains().facet_domains()](self.ind)

        # Find radius and center
        A = get_area(mesh, self.ind)
        center = get_center(mesh, self.ind)
        radius = np.sqrt(A/pi)

        # Get normal
        normal = get_average_normal(mesh, self.ind)
