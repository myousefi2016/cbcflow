from __future__ import division

__author__ = "Kristian Valen-Sendstad <kvs@simula.no>"
__date__ = "2008-02-01"
__copyright__ = "Copyright (C) 2008-2010 " + __author__
__license__  = "GNU GPL version 3 or any later version"

# Modified by Anders Logg, 2008-2010.

from ..core.nsscheme import *
from ..core.utils import Timer, epsilon, sigma
from ..core.timesteps import compute_regular_timesteps

# Hack:
def FacetDiameter(mesh):
    # FIXME: Should be actual facet diameter
    cell = mesh.ufl_cell()
    d = cell.d
    if d == 1:
        return 1
    if d == 2:
        return cell.facet_area
    if d == 3:
       return cell.circumradius

class PenaltyIPCS(NSScheme):
    "Incremental pressure-correction scheme with penalty terms for boundary conditions."

    def __init__(self, params=None):
        NSScheme.__init__(self, params, segregated=False)

    @classmethod
    def default_user_params(cls):
        params = ParamDict(
            penalty = ParamDict(
                gamma = 0.01,
                epsilon_small = 1e-4,
                epsilon_inf = 1e4,
                )
            )
        return params

    def xfetch_penalty_bc_terms(self, problem, V, Q, t):
        dummy, penalty_bcp = problem.penalty_boundary_conditions(V, Q, t)

        # Define Nitche discretization constants
        gamma = Constant(self.params.penalty.gamma)
        epsilon_small = Constant(self.params.penalty.epsilon_small)
        epsilon_inf = Constant(self.params.penalty.epsilon_inf)
        hE = Q.mesh().ufl_cell().max_facet_edge_length
        yh = gamma*hE
        eyhd = (epsilon_small + yh)
        eyhn = (epsilon_inf + yh)
        ds = problem.ds

        # Define trial and test functions
        p = TrialFunction(Q)
        q = TestFunction(Q)

        a = []
        L = []
        a_neumann = (
            -     yh / eyhn * (Dn(p)*q + p*Dn(q))
            +      1 / eyhn * (p*q)
            -   (epsilon_inf*yh) / eyhn * (Dn(p)*Dn(q))
            )
        a_dirichlet = (
            -  yh / eyhd * (Dn(p)*q + p*Dn(q))
            +  1 / eyhd * (p*q)
            - (epsilon_small*yh) / eyhd * (Dn(p)*Dn(q))
            )
        pbcs = []
        for (pbc, D) in penalty_bcp:
            pbcs += [pbc]
            a += [(a_dirichlet-a_neumann)*ds(D)]
            L += [(
                +       1 / eyhd * (pbc*q)
                -      yh / eyhd * (pbc*Dn(q))
                ) * ds(D)]
        if a:
            a += [a_neumann * ds()]
        return a, L, pbcs

    def fetch_penalty_bc_terms(self, problem, V, Q, t):
        dummy, penalty_bcp = problem.penalty_boundary_conditions(V, Q, t)

        # Define Nitche discretization constants
        gamma = Constant(self.params.penalty.gamma)
        epsilon_small = Constant(self.params.penalty.epsilon_small)
        epsilon_inf = Constant(self.params.penalty.epsilon_inf)
        #hE = Q.mesh().ufl_cell().max_facet_edge_length
        hE = Q.mesh().ufl_cell().facet_area
        yh = gamma*hE
        eyhd = (epsilon_small + yh)
        eyhn = (epsilon_inf + yh)
        ds = problem.ds

        # Define trial and test functions
        p = TrialFunction(Q)
        q = TestFunction(Q)

        a = []
        L = []
        a_neumann = (
            -     yh / eyhn * (Dn(p)*q + p*Dn(q))
            +      1 / eyhn * (p*q)
            -   (epsilon_inf*yh) / eyhn * (Dn(p)*Dn(q))
            )
        a_dirichlet = (
            -  yh / eyhd * (Dn(p)*q + p*Dn(q))
            +  1 / eyhd * (p*q)
            - (epsilon_small*yh) / eyhd * (Dn(p)*Dn(q))
            )
        pbcs = []
        for (pbc, D) in penalty_bcp:
            pbcs += [pbc]
            a += [(a_dirichlet-a_neumann)*ds(D)]
            L += [(
                +       1 / eyhd * (pbc*q)
                -      yh / eyhd * (pbc*Dn(q))
                ) * ds(D)]
        if a:
            a += [a_neumann * ds()]
        return a, L, pbcs

    def solve(self, problem, update):
        # Get problem parameters
        mesh = problem.mesh
        dt, timesteps = compute_regular_timesteps(problem)
        t = timesteps[0]
        dx = problem.dx
        ds = problem.ds

        # Define function spaces
        V = VectorFunctionSpace(mesh, "CG", self.params.u_degree)
        Q = FunctionSpace(mesh, "CG", self.params.p_degree)

        # Get initial conditions
        u0, p0 = problem.initial_conditions(V, Q)
        u0 = project(as_vector(u0), V)
        p0 = project(p0, Q)

        # Get boundary conditions
        bcu, bcp = self.fetch_bcs(problem, V, Q, t)
        #pbcu, pbcp = self.fetch_penalty_bcs(problem, V, Q)

        # Test and trial functions
        v = TestFunction(V)
        q = TestFunction(Q)
        u = TrialFunction(V)
        p = TrialFunction(Q)

        # Functions
        u1 = Function(V)
        p1 = Function(p0)

        nu = Constant(problem.params.mu/problem.params.rho)
        k  = Constant(dt)
        f  = as_vector(problem.body_force(V))
        n  = FacetNormal(mesh)

        # Tentative velocity step
        u_mean = 0.5 * (u + u0)
        u_diff = (u - u0)
        F_u_tent = ((1/k) * inner(v, u_diff) * dx()
                    + inner(v, grad(u0)*u0) * dx()
                    + inner(epsilon(v), sigma(u_mean, p0, nu)) * dx()
                    - nu * inner(grad(u_mean).T*n, v) * ds()
                    + inner(v, p0*n) * ds()
                    - inner(v, f) * dx())
        a_u_tent = lhs(F_u_tent)
        L_u_tent = rhs(F_u_tent)

        # Pressure correction
        a_p_corr = inner(grad(q), grad(p))*dx()
        L_p_corr = inner(grad(q), grad(p0))*dx() - (1/k)*q*div(u1)*dx()

        # Velocity correction
        a_u_corr = inner(v, u)*dx()
        L_u_corr = inner(v, u1)*dx() - k*inner(v, grad(p1-p0))*dx()

        # Add boundary condition penalty terms to tentative velocity forms
        if 0:
            abcs, Lbcs = pbcu
            for abc in abcs:
                a_u_tent += abc
            for Lbc in Lbcs:
                L_u_tent += Lbc

        # Add boundary condition penalty terms to pressure correction forms
        R = FunctionSpace(mesh, "Real", 0)
        tr = Function(R)
        tr.vector()[:] = t
        pbcp = self.fetch_penalty_bc_terms(problem, V, Q, tr)
        abcs, Lbcs, pbcs = pbcp
        for abc in abcs:
            a_p_corr += abc
        for Lbc in Lbcs:
            L_p_corr += Lbc

        # Add boundary condition penalty terms to velocity correction forms
        if 0:
            abcs, Lbcs = pbcu
            for abc in abcs:
                a_u_corr += abc
            for Lbc in Lbcs:
                L_u_corr += Lbc

        # Assemble matrices
        A_u_tent = assemble(a_u_tent)
        A_p_corr = assemble(a_p_corr)
        A_u_corr = assemble(a_u_corr)

        if self.params.solver_p:
            solver_p_params = self.params.solver_p
        elif len(bcp) == 0:
            solver_p_params = self.params.solver_p_neumann
        else:
            solver_p_params = self.params.solver_p_dirichlet

        # Time loop
        timer = Timer(self.params.enable_timer)
        for timestep in xrange(1,len(timesteps)):
            t = timesteps[timestep]
            tr.vector()[:] = t

            # DEBUG: Pressure looks good (around 0, 176):
            #print [assemble(pbcs[0]**2*ds(1), mesh=mesh), assemble(pbcs[1]**2*ds(2), mesh=mesh)]

            # Get boundary conditions
            bcu, bcp = self.fetch_bcs(problem, V, Q, t)
            timer.completed("update & fetch bc")

            # Compute tentative velocity step
            b = assemble(L_u_tent)
            for bc in bcu: bc.apply(A_u_tent, b)
            timer.completed("u1 construct rhs")
            print "u1 rhs norm:", norm(b)

            iter = solve(A_u_tent, u1.vector(), b, *self.params.solver_u_tent)
            timer.completed("u1 solve (%s, %d, %d)"%(', '.join(self.params.solver_u_tent), b.size(), iter))
            print "u1 norm:", norm(u1.vector())

            # Pressure correction
            b = assemble(L_p_corr)
            for bc in bcp: bc.apply(A_p_corr, b)
            timer.completed("p construct rhs")
            print "pressure rhs norm:", norm(b)

            iter = solve(A_p_corr, p1.vector(), b, *solver_p_params)
            timer.completed("p solve (%s, %d, %d)"%(', '.join(solver_p_params), b.size(), iter))
            print "pressure norm:", norm(p1.vector())

            # Velocity correction
            b = assemble(L_u_corr)
            for bc in bcu: bc.apply(A_u_corr, b)
            timer.completed("u2 construct rhs")
            print "u2 rhs norm:", norm(b)

            solver_params = self.params.solver_u_corr
            iter = solve(A_u_corr, u1.vector(), b, *solver_params)
            timer.completed("u2 solve (%s, %d, %d)"%(', '.join(solver_params), b.size(),iter))
            print "u2 norm:", norm(u1.vector())

            # Update postprocessing
            update(u1, p1, t, timestep)

            # Rotate functions for next timestep
            u0.assign(u1)
            p0.assign(p1)
