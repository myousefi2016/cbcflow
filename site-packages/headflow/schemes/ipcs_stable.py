...
class IPCS_Stable(NSScheme):
    ...
    def solve(self, problem, update, restart=None):
        ...
        _u0, _p0 = problem.initial_conditions(V, Q)
        u0 = as_vector([project(_, V) for _ in _u0]) # u^{n-1}
        u1 = as_vector([project(_, V) for _ in _u0]) # u^n
        ...
        # Functions
        u2 = as_vector([Function(V) for d in dims]) # u^{n+1}
        p2 = Function(Q)                   # p^{n+1}
