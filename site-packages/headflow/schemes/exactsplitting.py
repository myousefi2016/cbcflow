...
class ExactSplitting(NSScheme):
    ...
    def solve(self, problem, update):
        ...
        for timestep in xrange(1,len(timesteps)):
            ...
            Aprec = DD_ILU(A)
            A_inv1 = LGMRES(A, precond=Aprec, initial_guess=u1.vector(), maxiter=250,
                            nonconvergence_is_fatal=True, tolerance=self.params.conv_diff_tolerance)
            u1.vector()[:] = A_inv1*bb0

            S_inv = LGMRES(S, precond=X_inv, tolerance=self.params.schur_tolerance, maxiter=100,
                           show=1, nonconvergence_is_fatal=True)
            p1.vector()[:] = -S_inv * (bb1 - D * u1.vector())

            A_inv2 = LGMRES(A, precond=Aprec, initial_guess=u_corr.vector(), maxiter=250,
                            nonconvergence_is_fatal=True, tolerance=self.params.conv_diff_tolerance)
