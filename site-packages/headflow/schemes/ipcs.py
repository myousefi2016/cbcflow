...
class IPCS(NSScheme):
    ...
    def solve(self, problem, update):
        ...
        for timestep, t in enumerate(t_range):
            ...
            iter = solve(A_u_tent, u1.vector(), b, *solver_params)
            self.timer("u1 solve (%s, %d, %d)"%(', '.join(solver), b.size(), iter))

            # Pressure correction
            b = assemble(L_p_corr)
            if len(bcp) == 0 or is_periodic(bcp):
                solver_params = self.params.solver_p or self.params.solver_p_neumann
                normalize(b)
            else:
                solver_params = self.params.solver_p or self.params.solver_p_dirichlet
            for bc in bcp: bc.apply(A_p_corr, b)
            self.timer("p construct rhs")

            iter = solve(A_p_corr, p1.vector(), b, *solver_params)
