...
class CoupledPreconditoned(NSScheme):
    ...
    def solve(self, problem, update):
        assert block is not None, "Failed to import cbc.block."
        ...
        # Preconditioner   
        Lp = Constant(0)*q*dx()
        Ap, _ = assemble_system(ap, Lp, bcp)
        Fp = assemble(fp)
        Mp = assemble(mp)
...
#        P_inv.scheme('sgs')
        ...
        for timestep in xrange(1,len(timesteps)):
            ...
            t1 = time()
            TOL = 1e-6
            AAinv = LGMRES(AA, precond=P_inv, tolerance=TOL, maxiter=250, show=1, initial_guess=guess)
