...
def make_mixed_bcs(problem, spaces, bcs, v):
    ...
    if 0:
        Lbc = -sum(dot(Constant(region)*n, v)*ds(region) for (function, region) in bcp_raw)
        print Lbc
        for itg in Lbc.integrals():
            print
            print itg
            print str(itg.integrand())
        for function, region in bcp_raw:
            plot(function, mesh=problem.mesh, title="region %d"%region)
        interactive()

    if 0:
        print
        print '\n'.join([str(itg.integrand()) for itg in Lbc.integrals()])
        print
        print '\n'.join([repr(itg.integrand()) for itg in Lbc.integrals()])
        print
        print '\n'.join([itg.integrand()._repr_latex_() for itg in Lbc.integrals()])
        print
        crash
    return bcu, Lbc
...
class CoupledPicard(NSOptScheme):
    ...
    def solve(self, problem, update):
        ...
        if 0:
            ...
            solve(dot(u,v)*dx() == Lbc, d)
            assemble(Lbc, tensor=b.vector())
            c = project(d, V1)

            File("b.pvd") << b
            File("b.xml.gz") << b.vector()
            print max(d.vector().array()[:]**2)
            print max(c.vector().array()[:]**2)
            print max(b.vector().array()[:]**2)
            print sum(d.vector().array()[:]**2)
            print sum(c.vector().array()[:]**2)
            print sum(b.vector().array()[:]**2)
            plot(d)
            plot(b)
            plot(c, title='c')
            interactive()
        ...
        print
        print solver.parameters["lu_solver"]["report"]
        print solver.parameters["newton_solver"]["report"]
        print solver.parameters["newton_solver"].items()
        print
        ...
        for k in solver.parameters.keys():
            print
            print k
            p = solver.parameters[k]
            if hasattr(p, 'keys'):
                print p.keys()
            else:
                print p
