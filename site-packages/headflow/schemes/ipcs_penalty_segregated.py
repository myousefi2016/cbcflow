...
class SegregatedPenaltyIPCS(NSScheme):
    ...
    def solve(self, problem, update, restart=None):
        ...
        if 0:
            # FIXME: Add back option to project initial condition to divergence free space?
            ...
            solver.set_transpose_nullspace([null_space]) # FIXME: New dolfin approach
            dsw = ds(problem.wall_boundary_markers) # FIXME: Standardize markers
            # ... solve for lagrange multiplier
            solver.solve(pc.vector(), assemble(div(uinit)*q*dx - dot(n,uinit)*q*dsw))
            # ... update uinit
            uinit = uinit + grad(pc)
            # Store new initial conditions in ics tuple
            ics = uinit, pinit
