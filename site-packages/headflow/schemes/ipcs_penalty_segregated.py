...
class SegregatedPenaltyIPCS(NSScheme):
    ...
    def solve(self, problem, update, restart=None):
        ...
        V = spaces.U # FIXME: Rename V -> U below
        Q = spaces.Q
        ...
        for d in dims: u1.assign(u0[d])
        p1.assign(p0)

        if 0: # FIXME: Add back option to project initial condition to divergence free space?
            # Get initial conditions
            uinit, pinit = problem.initial_conditions(V, Q, controls)
            uinit = as_vector(uinit)

            # FIXME: Make this an option?
            # Project initial condition into divergence free space
            # Create null space basis and attach to Krylov solver ...
            Q2 = FunctionSpace(mesh, "CG", self.params.p_degree)
            pc = Function(Q2, name="pcuinit")
            null_space = Vector(pc.vector())
            Q2.dofmap().set(null_space, 1.0)
            A = assemble(dot(grad(p), grad(q))*dx)
            solver = KrylovSolver(A, "gmres")
            solver.set_nullspace([null_space])
            solver.set_transpose_nullspace([null_space])
            dsw = ds(0) # FIXME: This should be at the wall, BCs here are a bit fishy though
            # ... solve for lagrange multiplier
            solver.solve(pc.vector(), assemble(div(uinit)*q*dx - dot(n,uinit)*q*dsw))
            # ... update uinit
            uinit = uinit + grad(pc)

            # Apply initial conditions
            for i in xrange(d): u0[i].assign(project(uinit[i], V))
