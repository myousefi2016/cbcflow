...
class Karper(NSScheme):
    ...
    def solve(self, problem, update):
        ...
        if 0:
            print 'ca'
            print ":", assemble(cell_avg(u0)**2*dx())
            print ":", assemble(cell_avg(u0('+'))**2*dS())
            print ":", assemble(cell_avg(u0('-'))**2*dS())
            print ":", assemble((cell_avg(u0('+'))-cell_avg(u0('-')))**2*dS())
            print ":", assemble(cell_avg(u0)**2*ds())
            print 'fa'
            print ":", assemble(facet_avg(u0('+'))**2*dS())
            print ":", assemble(facet_avg(u0('-'))**2*dS())
            print ":", assemble((facet_avg(u0('+'))-facet_avg(u0('-')))**2*dS())
            print ":", assemble(facet_avg(u0)**2*ds())
            print 'c'
            print ":", assemble(Constant(1.0)('+')*dS(), mesh=mesh)
            print ":", assemble(conditional(gt(dot(facet_avg(u0('+')), n('+')), 0.0), 1.0,0.0)*dS())
            print ":", assemble(conditional(gt(dot(facet_avg(u0('-')), n('-')), 0.0), 1.0,0.0)*dS())
            print ":", assemble(conditional(And(gt(dot(facet_avg(u0('-')), n('-')), 0.0),
                                                gt(dot(facet_avg(u0('+')), n('+')), 0.0)), 1.0, 0.0)*dS())
        Ups = []
        Ups.append(
            # From + side:
              Max(dot(facet_avg(u0('+')), n('+')), 0.0) * cell_avg(u1('+'))
            + Max(dot(facet_avg(u0('-')), n('-')), 0.0) * cell_avg(u1('-'))
            )

        Ups.append(
            # From + side:
              Max(dot(facet_avg(u0('-')), n('+')), 0.0) * cell_avg(u1('+')) # A
            + Min(dot(facet_avg(u0('+')), n('+')), 0.0) * cell_avg(u1('-')) # B
            # From - side:
            + Max(dot(facet_avg(u0('+')), n('-')), 0.0) * cell_avg(u1('-')) # -B
            + Min(dot(facet_avg(u0('-')), n('-')), 0.0) * cell_avg(u1('+')) # -A
            )
        Ups.append(
            # From + side:
              Max(dot(facet_avg(u0('-')), n('+')), 0.0) * cell_avg(u1('+')) # A
        ...
        Ups.append(
             Min(dot(facet_avg(u0('+')), n('+')), 0.0) * cell_avg(u1('-'))
              )
        Ups.append(
             Max(dot(facet_avg(u0('+')), n('-')), 0.0) * cell_avg(u1('-'))
              )
        ...
        for nn in n('+'), n('-'):
            Ups.append(
                  Max(dot(facet_avg(u0('+')), nn), 0.0) * cell_avg(u1('-'))
                - Max(dot(facet_avg(u0('-')), nn), 0.0) * cell_avg(u1('+'))
                )
            Ups.append(
                  Max(dot(facet_avg(u0('+')), nn), 0.0) * cell_avg(u1('+'))
        ...
        Up = (
              Max(dot(facet_avg(u0('+')), n('+')), 0.0) * cell_avg(u('-'))
            + Max(dot(facet_avg(u0('-')), n('-')), 0.0) * cell_avg(u('+'))
            )
        Upe = (
              Max(dot(facet_avg(u0), n), 0.0) * cell_avg(u)
            )
        bnd = -k*dot(Upe, cell_avg(v))*ds()

        a = (
           #dot(cell_avg(u), cell_avg(v))*dx()
            dot(cell_avg(u), v)*dx()
