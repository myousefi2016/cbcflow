...
class Karper(NSScheme):
    ...
    def solve(self, problem, update):
        ...
        def cell_avg(function):
            "FIXME: Implement this operator in UFL and FFC and UFLACS."
            return function

        # Variational forms
        # FIXME: Check signs and directions of DG terms
        # FIXME: Should the laplace part be dx or dS?
        kp = k('+')
        nup = nu('+')
        a = (
            dot(cell_avg(u), cell_avg(v))*dx()
            - kp*dot(Max(dot(u0('-'), n('+')), 0.0) * cell_avg(u('+'))
                  + Max(dot(u0('+'), n('+')), 0.0) * cell_avg(u('-')),
                  cell_avg(v('+')) - cell_avg(v('-'))
                  )*dS()
            #+ k*nu*inner(grad(u), grad(v))*dx()
            + kp*nup*inner(grad(avg(u)),grad(avg(v)))*dS()
            - k*p*div(v)*dx()
            + k*div(u)*q*dx()
            )
        L = dot(cell_avg(u0), cell_avg(v))*dx() + k*dot(f,v)*dx() + k*Lbc
