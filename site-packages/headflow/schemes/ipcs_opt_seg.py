...
from ..core.utils import is_periodic, epsilon

class IPCS_opt_seg(NSScheme):
    ...
    def solve(self, problem, restart=None):
        assert problem.params.segregated == True
        ...
        for t in t_range:
            ...
            for d, S, rhs, u1_comp, bcu_comp in zip(dims, solver_u_tent, rhs_u_tent, u1, bcu):
                ...
                self.timer("u0 solve (%s, %d, %d)"%(', '.join(self.params.solver_u_tent), A.size(0), iter))
            ...
            for S, rhs, u1_comp, bcu_comp in zip(solver_u_corr, rhs_u_corr, u1, bcu):
                ...
                iter = S.solve(u1_comp.vector(), b)
                self.timer("u1 solve (%s, %d, %d)"%(', '.join(self.params.solver_u_corr), A.size(0),iter))
