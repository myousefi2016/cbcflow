...
class BottiPietro(NSScheme):
    ...
    def solve(self, problem, update):
        ...
        spaces = NSSpacePoolSplit(mesh, self.params.u_degree, self.params.p_degree, u_family="DG") # FIXME
        V = spaces.V
        Q = spaces.Q
        ...
        ubcs, _pbcs = bcs
        ubcs = [(as_vector(function), region) for function, domain in ubcs]
        ...
        for ubc, region in ubcs:
            if 0: # FIXME: What is the right formulation for this term?
                ...
                b_u += pen_ext*dot(ubc,n)*dot(v,n)*dsb(region)
        ...
        for timestep in xrange(1,len(timesteps)):
            ...
            bh_u = assemble(b_u)
            solve(A_u, u1.vector(), bh_u, solver_name, solver_parameters=solver_params)

            # Solve pressure equation
            p0.assign(p1)
            bh_p = assemble(b_p)
            for bc in bcp: bc.apply(A_p, bh_p)
            solve(A_p, p1.vector(), bh_p) # -> pn+1
