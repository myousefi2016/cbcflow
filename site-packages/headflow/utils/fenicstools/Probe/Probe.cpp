   ...
   _element(V.element()), _num_evals(0)
{
  ...
  //boost::shared_ptr<const dolfin::MeshPointIntersection> intersection = intersect(mesh, point);
  //std::vector<unsigned int> idvector = intersect(mesh, point)->intersected_cells();
  ...
  unsigned int cell_index = tree->compute_first_entity_collision(point);
   
  // int id = intersect(mesh, point);
  //int id = 1;
  ...
  if (cell_index != std::numeric_limits<unsigned int>::max())
  {
    ...
    dolfin_cell = new Cell(mesh, cell_index);
